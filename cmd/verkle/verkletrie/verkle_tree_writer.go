...
func (v *VerkleTreeWriter) CommitVerkleTreeFromScratch() (common.Hash, error) {
	...
	return root.Commit().Bytes(), verkleCollector.Load(v.db, kv.VerkleTrie, etl.IdentityLoadFunc, etl.TransformArgs{Quit: context.Background().Done(),
...
func (v *VerkleTreeWriter) CommitVerkleTree(root common.Hash) (common.Hash, error) {
	...
	commitment := rootNode.Commit().Bytes()
